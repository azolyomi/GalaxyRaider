...
function addPingRole(msg, args) {
  if (!CONFIG.SystemConfig.servers[msg.guildID])
    return "Run the `.config` command first.";
  else if (!(msg.roleMentions.length > 0))
    return "You need to mention at least one role for that!";

  let pingtype = args[0];
  let acceptablepingtypes = [
    "void",
    "cult",
    "shatters",
    "nest",
  ...
  if (!acceptablepingtypes.includes(pingtype))
    return `You must specify which type of run this role will be pinged for. Acceptable types: \`${acceptablepingtypes.join(
      ", "
    )}\``;

  msg.roleMentions.forEach((id) => {
    if (!CONFIG.SystemConfig.servers[msg.guildID].pings[pingtype].includes(id))
      CONFIG.SystemConfig.servers[msg.guildID].pings[pingtype].push(id);
  });

  CONFIG.updateConfig(msg.guildID);
  return `Successfully added ${msg.roleMentions
    .map((id) => `<@&${id}>`)
    .join(", ")} to the bot's list of \`${pingtype}\` pinged roles.`;
}
...
function deletePingRole(msg, args) {
  ...
  if (!acceptablepingtypes.includes(pingtype))
    return `You must specify which type of run this role will have the ping removed for. Acceptable types: \`${acceptablepingtypes.join(
      ", "
    )}\``;

  msg.roleMentions.forEach((id) => {
    CONFIG.SystemConfig.servers[msg.guildID].pings[pingtype] =
      CONFIG.SystemConfig.servers[msg.guildID].pings[pingtype].filter(
        (roleid) => roleid != id
      );
  });

  CONFIG.updateConfig(msg.guildID);
  return `Successfully removed ${msg.roleMentions
    .map((id) => `<@&${id}>`)
    .join(", ")} from the bot's list of \`${pingtype}\` pinged roles.`;
}

exports.addPingRole = addPingRole;
exports.deletePingRole = deletePingRole;

exports.helpCommand = `Ping Role Command
...
function pingReacted(msg, member, emoji, added) {
  if (RAIDCONSTANTS.whitebagEmoji.includes(emoji.id)) {
    CONFIG.SystemConfig.servers[msg.guildID].pings.void.forEach(async (id) => {
      try {
        added
          ? member.addRole(id, "ping assignment")
          : (
              await CONSTANTS.bot.getRESTGuildMember(msg.guildID, member)
            ).removeRole(id, "ping assignment");
      } catch (e) {}
    });
    ...
    CONFIG.SystemConfig.servers[msg.guildID].pings.shatters.forEach(
      async (id) => {
        try {
          added
            ? member.addRole(id, "ping assignment")
    ...
    CONFIG.SystemConfig.servers[msg.guildID].pings.fungal.forEach(
      async (id) => {
        try {
          added
            ? member.addRole(id, "ping assignment")
    ...
    CONFIG.SystemConfig.servers[msg.guildID].pings.steamworks.forEach(
      async (id) => {
        try {
          added
            ? member.addRole(id, "ping assignment")
  ...
  } else if (RAIDCONSTANTS.steamworksEmoji.includes(emoji.id)) {
    CONFIG.SystemConfig.servers[msg.guildID].pings.steamworks.forEach(
      async (id) => {
        try {
          added
...
async function setupPingMessage(msg, args) {
  ...
  let pingmessagedescription = `**__Important Information__**
  ...
  if (CONFIG.SystemConfig.servers[msg.guildID].pings.steamworks.length > 0) {
    pingmessagedescription += `React with the ${RAIDCONSTANTS.steamworksEmoji} if you would like to be pinged for \`kogbold steamworks\` runs.\n`;
    pingreactions.push(RAIDCONSTANTS.steamworksReaction);
  }
...
exports.setupPingMessageHelpCommand = `Setup Ping Message Command
