...
async function makevc(message, args) {
	if (args.length < 2)
		return "You have to specify a `Category` and a `Type`! Example: `makevc main regular SpaceRaider's O3`";
	//VC Category Handling
	let category = args.shift();
	if (!acceptableCategories.includes(category))
		return `Acceptable categories: \`${acceptableCategories.join(", ")}\``;
	else if (
		category === "veteran" &&
		!CONFIG.SystemConfig.servers[message.guildID].afkaccess.vethalls.some((item) =>
			message.member.roles.includes(item)
		) &&
		!CONFIG.SystemConfig.servers[message.guildID].afkaccess.vetoryx.some((item) =>
			message.member.roles.includes(item)
		) &&
		!CONFIG.SystemConfig.servers[message.guildID].afkaccess.vetexaltation.some((item) =>
			message.member.roles.includes(item)
		) &&
		!CONFIG.SystemConfig.servers[message.guildID].afkaccess.vetmisc.some((item) =>
			message.member.roles.includes(item)
		)
	)
		return "You need a `Veteran Leader Role` to do that.";

	let CHANNELOBJECT;
	if (category === "main")
		CHANNELOBJECT = CONFIG.SystemConfig.servers[message.guildID].channels.Main;
